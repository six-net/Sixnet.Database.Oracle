...
namespace EZNEW.Data.Oracle
{
    ...
    internal class OracleQueryTranslator : IQueryTranslator
    {
        ...
        public QueryTranslationResult ExecuteTranslation(IQuery query, QueryLocation location, CommandParameters parameters = null, string objectName = "", bool useSort = true)
        {
            ...
            if (query.ExecutionMode == QueryExecutionMode.QueryObject)
            {
                ...
                if (useSort && !query.Sorts.IsNullOrEmpty())
                {
                    ...
                    {
                        string fieldName = ConvertSortFieldName(query, objectName, sortEntry);
                        sortBuilder.Append($"{fieldName} {(sortEntry.Desc ? DescKeyWord : AscKeyWord)},");
                ...
                if (!query.Joins.IsNullOrEmpty())
                {
                    foreach (var joinEntry in query.Joins)
                    {
                        ...
                        string joinQueryObjectName = DataAccessContext.GetJoinEntityObjectName(joinEntry.JoinObjectFilter);
                        if (string.IsNullOrWhiteSpace(joinQueryResult.CombineScript))
                        {
                            var joinResult = GetJoinCondition(query, joinEntry, parameters, conditionObjectName, joinObjName);
                            ...
                            joinBuilder.Append($" {GetJoinOperator(joinEntry.Type)} {OracleManager.FormatTableName(joinQueryObjectName)} {joinObjName}{joinConnection}");
                        ...
                        else
                        {
                            ...
                            var joinResult = GetJoinCondition(query, joinEntry, parameters, conditionObjectName, combineJoinObjName);
                            if (!joinResult.WithScripts.IsNullOrEmpty())
                            {
                                withScripts.AddRange(joinResult.WithScripts);
                                recurveTableName = joinResult.RecurveObjectName;
                            ...
                            string joinConnection = joinResult.ConditionString;
                            string combineQueryFields = string.Join(",", OracleManager.FormatQueryFields(joinObjName, joinEntry.JoinObjectFilter, joinQueryEntityType, true, false));
                            ...
                            joinBuilder.Append($" {GetJoinOperator(joinEntry.Type)} (SELECT {combineQueryFields} FROM {OracleManager.FormatTableName(joinQueryObjectName)} {joinObjName} {(joinQueryResult.AllowJoin ? joinQueryResult.JoinScript : string.Empty)} {combineCondition} {joinQueryResult.CombineScript}) {combineJoinObjName}{joinConnection}");
                        }
        ...
        QueryTranslationResult TranslateSubquery(IQuery subquery, CommandParameters parameters, string criterionFieldName, string sqlOperator)
        {
            ...
            string subqueryQueryField = $"{subqueryObjectPetName}.{subqueryFieldName}";
            string subquerySortFieldString = string.Empty;
            if (!subqueryTranslationResult.SortFields.IsNullOrEmpty())
            {
                subqueryTranslationResult.SortFields.Remove(subqueryQueryField);
                if (!subqueryTranslationResult.SortFields.IsNullOrEmpty())
                {
                    subquerySortFieldString = $",{string.Join(",", subqueryTranslationResult.SortFields)}";
                }
            }
            if (subqueryLimitResult.Item1) //use wapper
            {
                if (hasCombine)
                {
                    subqueryCondition = userSort && hasSort
                        ? $"{criterionFieldName} {sqlOperator} (SELECT S{subqueryQueryField} FROM (SELECT {subqueryQueryField} FROM (SELECT {subqueryQueryField} FROM (SELECT {subqueryQueryField}{subquerySortFieldString} FROM {subqueryObjectName} {subqueryObjectPetName} {joinScript} {conditionString} {subqueryTranslationResult.CombineScript}) {subqueryObjectPetName} {subquerySortString}) {subqueryObjectPetName} {limitString}) S{subqueryObjectPetName})"
                        : $"{criterionFieldName} {sqlOperator} (SELECT S{subqueryQueryField} FROM (SELECT {subqueryQueryField} FROM (SELECT {subqueryQueryField}{subquerySortFieldString} FROM {subqueryObjectName} {subqueryObjectPetName} {joinScript} {conditionString} {subqueryTranslationResult.CombineScript}) {subqueryObjectPetName} {limitString} {subquerySortString}) S{subqueryObjectPetName})";
                }
                else
                {
                    subqueryCondition = userSort && hasSort
                        ? $"{criterionFieldName} {sqlOperator} (SELECT S{subqueryQueryField} FROM (SELECT {subqueryQueryField} FROM (SELECT {subqueryQueryField} FROM {subqueryObjectName} {subqueryObjectPetName} {joinScript} {conditionString} {subquerySortString}) {subqueryObjectPetName} {limitString}) S{subqueryObjectPetName})"
                        : $"{criterionFieldName} {sqlOperator} (SELECT S{subqueryQueryField} FROM (SELECT {subqueryQueryField} FROM {subqueryObjectName} {subqueryObjectPetName} {joinScript} {conditionString} {subquerySortString}) S{subqueryObjectPetName})";
                }
            }
            else
            {
                ...
                {
                    subqueryCondition = userSort && hasSort
                        ? $"{criterionFieldName} {sqlOperator} (SELECT {subqueryQueryField} FROM (SELECT {subqueryQueryField} FROM (SELECT {subqueryQueryField}{subquerySortFieldString} FROM {subqueryObjectName} {subqueryObjectPetName} {joinScript} {conditionString} {subqueryTranslationResult.CombineScript}) {subqueryObjectPetName} {subquerySortString}) {subqueryObjectPetName} {limitString})"
                        : $"{criterionFieldName} {sqlOperator} (SELECT {subqueryQueryField} FROM (SELECT {subqueryQueryField}{subquerySortFieldString} FROM {subqueryObjectName} {subqueryObjectPetName} {joinScript} {conditionString} {subqueryTranslationResult.CombineScript}) {subqueryObjectPetName} {limitString} {subquerySortString})";
                }
                else
                {
                    subqueryCondition = userSort && hasSort
                        ? $"{criterionFieldName} {sqlOperator} (SELECT {subqueryQueryField} FROM (SELECT {subqueryQueryField} FROM {subqueryObjectName} {subqueryObjectPetName} {joinScript} {conditionString} {subquerySortString}) {subqueryObjectPetName} {limitString})"
                        : $"{criterionFieldName} {sqlOperator} (SELECT {subqueryQueryField} FROM {subqueryObjectName} {subqueryObjectPetName} {joinScript} {conditionString} {subquerySortString})";
                }
            }
            ...
            foreach (var criterion in joinCriteria)
            {
                if (criterion is RegularJoinCriterion regularJoinCriterion)
                {
                    ...
                    else
                    {
                        ...
                        joinList.Add($"{criterionConnector}{joinFieldName} {sqlOperator} {OracleManager.ParameterPrefix}{parameterName}");
                    }
        ...
        string ConvertFieldName(IQuery query, string objectName, FieldInfo fieldInfo)
        {
            ...
            var fieldConversionResult = OracleManager.ConvertField(DatabaseServer, fieldInfo.ConversionOptions, objectName, fieldName);
